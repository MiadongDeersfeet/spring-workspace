...
public class BoardServiceImpl implements BoardService {
	...
	*/
	...
	public Map<String, Object> findAll(Long page) {
		...
		// 뭐할까용?? ==> 여기서 뭐할까용?? 유효성 검증 장난꾸러기들이 boards/page=-1 , page=0 이렇게 할 수 있잖아요~!
		if(page < 1) {
		...
		map.put("pi", pi);
		map.put("boards", boards); // 지역변수라서 못 담아요.
		return map;
	}
	...
	private void validateContent(BoardDTO board) {
		...
		if(board.getBoardTitle().contains("김윤기바보")) {
			board.getBoardTitle().replaceAll("김윤기바보", "글쓴사람바보");
		}
	...
	public int save(BoardDTO board, MultipartFile upfile, HttpSession session) {
		...
		// 3번 파일이 있을 경우 업로드 (빈 문자열이 아니여야해요) ==> 빈으로 분리하기 숙제
	...
	public int insertReply(ReplyDTO reply, HttpSession session) {
		...
		MemberDTO loginMember = ((MemberDTO)session.getAttribute("loginMember"));
		if(loginMember == null) {
			throw new AuthenticationException("이 장면을 꿈에서 본 것 같은데");
		}
		//유효성 검증
